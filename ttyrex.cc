...
int main()
{
    ...
    if (error) {
        std::cout << "ERROR::FREETYPE: Failed to load font: " << FT_Error_String(error) << std::endl;
        return -1;
    }
        ...
        if (face->glyph->bitmap.pixel_mode == FT_PIXEL_MODE_MONO) {
            ...
            for (int row = 0; row < static_cast<int>(bitmap.rows); ++row) {
                for (int pixel = 0; pixel < static_cast<int>(bitmap.width); ++pixel) {
                    uint8_t buffer_byte = bitmap.buffer[row * bitmap.pitch + (pixel >> 3)];
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        RenderText(shader, "(C) LearnOpenGL.com", 540.0f, 570.0f, 1.0f, glm::vec3(0.3, 0.7f, 0.9f));
